...
This is done with Maximum Likelihood (ML) using Least Squares
...
pois1<-as_tibble(y=y,u=u,x=x) %>% 
ggplot()+
  geom_line(aes(x=x,y=y),linewidth=2,colour="darkcyan",alpha=0.7)+
  geom_point(aes(x=x,y=u),size=2,colour="grey50")+
  theme_Bede()

pois2<-as_tibble(y=y,u=u,x=x) %>% 
ggplot()+
...
binom2<-as_tibble(y=y,u=u,x=x) %>% 
ggplot()+
  geom_line(aes(x=x,y=log(y/(1-y))),linewidth=2,colour="darkcyan",alpha=0.7)+
  geom_point(aes(x=x,y=log(u/(1-u))),size=2,colour="grey50")+
  geom_segment(aes(x=x,xend=x,y=log(y/(1-y)),yend=log(u/(1-u))),colour="red")+
  theme_Bede()
